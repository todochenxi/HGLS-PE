...
def generate_graph(data, time, time_num, nodes_num=None, rel_nums=None, name='Sample'):
    ...
    rel_o = []
    L = len(time) # 总的时间长度
    time_num = np.roll(time_num, 1)  # 沿着轴向前滚动一次, 如 1，2，3，4－>4,1,2,3
    time_num[0] = 0
    idx = np.cumsum(time_num)
    num_t = np.zeros(L) # 记录每个时刻的？？？
    for i, tim in enumerate(time):
        ...
        uniq_v, edges = np.unique((src1, dst1), return_inverse=True)  # 返回额外的索引edges，uniq_v 是一个节点
        n_src1, n_dst1 = np.reshape(edges, (2, -1)) + idx[i]  # 给节点在新图中重新编码
        u.append(np.concatenate((n_src1, n_dst1)))  # 保存新的边
        u_id.append(uniq_v)  # 保存 节点
        t_id.append(i*np.ones(len(uniq_v), dtype=int)) # 为每一个uniq_v 分配一个当前时间的time_id
        v.append(np.concatenate((n_dst1, n_src1)))  # reverse_edges
        rel.append(np.concatenate((rel1, rel1+rel_nums)))  # 编码新的关系索引
        rel_1.append(np.concatenate((rel1, rel1+rel_nums)))  # 也是编码新的关系索引
        rel_r.append(0 * np.ones(len(rel1) * 2, dtype=int))  # 元素为0，长度为2＊len(rel)的数组
        rel_t.append(i*np.ones(len(rel1)*2, dtype=int))   # 为当前时间点的关系分配一个时间id
        rel_s.append(i*np.ones(len(rel1)*2, dtype=int))  # 一样
        rel_s_.append(i*np.ones(len(rel1)*2, dtype=int))  # 一样
        rel_h.append(np.concatenate((n_src1, n_dst1)))  # save new edges
        rel_o.append(np.concatenate((n_dst1, n_src1)))  # save new reverse edges
        ...
        for j, tim in enumerate(time[i+1:]):
            ...
            u.append(np.concatenate((u1 + idx[i], u2 + idx[i+j+1])))    #  虚拟边
            v.append(np.concatenate((u2 + idx[i+j+1], u1 + idx[i])))    # 虚拟 反向边
...
def generate_graph_reverse(data, time, time_num, nodes_num=None, rel_nums=None, name='Sample'):
    """
    :param data:
    :param time:
    :param time_num: 每个时刻下entity的数量
    ...
    for i, tim in enumerate(time):
        ...
        if i == 0:
            continue
        for j, tim in enumerate(time[:i]):
            if name == 'Sample':
                data_2 = get_data_with_t(data, time[j])
            else:
                data_2 = data[j]
            src2, rel2, dst2 = data_2[:,[0,1,2]].transpose()
            uniq_v2, edges2 = np.unique((src2, dst2), return_inverse=True)
            ...
            u1 = np.where(np.in1d(uniq_v, un_entity))[0]    # 判断公共节点在不同子图内的索引
            u2 = np.where(np.in1d(uniq_v2, un_entity))[0]   # 判断公共节点在不同子图内的索引
            u.append(np.concatenate((u1 + idx[i], u2 + idx[j])))    #  虚拟边
            v.append(np.concatenate((u2 + idx[j], u1 + idx[i])))    # 虚拟 反向边
            rel.append(2*rel_nums*np.ones(2*len(un_entity), dtype=int))               # 时间类型的边
            rel_1.append((2 * rel_nums + j) * np.ones(2 * len(un_entity), dtype=int)) # 给不同时间类型的边定义大小
            #rel.append((2 * rel_nums + j) * np.ones(2 * len(un_entity), dtype=int))  # 时间差
            rel_r.append((i-j) * np.ones(2 * len(un_entity), dtype=int))               # 时间差
            rel_t.append(L*np.ones(2*len(un_entity), dtype=int))
            rel_s.append((j)*np.ones(2*len(un_entity), dtype=int))               #  头实体发生时间
            rel_s_.append(i*np.ones(2*len(un_entity), dtype=int))                    #  尾实体发生时间
            rel_h.append(np.concatenate((u1 + idx[i], u2 + idx[j])))
            rel_o.append(np.concatenate((u2 + idx[j], u1 + idx[i])))
    u = np.concatenate(u)
    u_id = np.concatenate(u_id)
    t_id = np.concatenate(t_id)
    v = np.concatenate(v)
    ...
    graph.ndata['t_id'] = torch.from_numpy(t_id).long()
    # ----计算每个时刻的s前一时刻在大图中的位置
    s_his = defaultdict(int)    #记录前一次的索引  记录每个节点最后一次交互的索引
    s_his_t = defaultdict(int)  #记录前一次发生的时间  记录每个节点最后一次交互发生的时间
    s_his_f = defaultdict(int)  #记录前一次的索引  记录每个节点最后一次交互的目标节点索引
    s_his_l = defaultdict(int)  #记录历史序列的长度   记录每个节点的历史交互次数
    s_last_index = np.zeros((nodes_num, L), dtype=int)   #记录每个时刻节点的索引
    s_last_t = L*np.ones((nodes_num, L), dtype=int)  # 记录每个时刻上次交互的时间,上次没有交互的节点会记为L
    s_last_f = graph.num_nodes() * np.ones((nodes_num, L), dtype=int) #记录每个时刻节点的的索引，没有交互过的节点值为nodes_num
    s_last_l = np.zeros((nodes_num, L), dtype=int)  # 记录每个时刻节点的历史交互次数
    node_id = graph.ndata['id'].numpy()  
    time_id = graph.ndata['t_id'].numpy()
    # node_id_new = len(node_id) - 1 - np.unique(node_id[::-1], return_index=True)[1]
    # 初始化
    id, node_id_f = np.unique(node_id, return_index=True)
    for n_i, ei in enumerate(id):
        s_his[ei] = node_id_f[n_i]  # 将该节点的前一次交互设置为该节点首次出现的位置
        s_his_t[ei] = L             # 将该节点的前一次交互的时间设置为L，表示它没有历史交互
        s_his_f[ei] = graph.num_nodes()  # 设置前一次交互的目标节点为图中的所有节点（即没有交互）
        s_his_l[ei] = 0                # 0  # 设置该节点的历史交互次数为0
    s_last_index[id, 0] = node_id_f  # 初始化 s_last_index 中每个节点在第0时刻的索引为它们首次出现的位置
    for i, tim in enumerate(time):
        if name == 'Sample':
            data_1 = get_data_with_t(data, tim)
        else:
...
def generate_graph_all(data, time, time_num, nodes_num=None, rel_nums=None, name='Sample'):
    """
    :param data:
    :param time:
    :param time_num: 每个时刻下entity的数量
    ...
    for i, tim in enumerate(time):
        ...
        # i =0 or i = len(time) -1 
        for j, tim in enumerate(list(time[:i])+list(time[i+1:])):
            # print(f"i = {i}, j={j}")
            if name == 'Sample':
                data_2 = get_data_with_t(data, time[i+j+1])
            ...
            rel_1.append((2 * rel_nums + j) * np.ones(2 * len(un_entity), dtype=int)) # 给不同时间类型的边定义大小
            #rel.append((2 * rel_nums + j) * np.ones(2 * len(un_entity), dtype=int))  # 时间差
            rel_r.append(abs(i-j) * np.ones(2 * len(un_entity), dtype=int))               # 时间差
            rel_t.append(L*np.ones(2*len(un_entity), dtype=int))
            rel_s.append((j)*np.ones(2*len(un_entity), dtype=int))               #  头实体发生时间
            rel_s_.append(i*np.ones(2*len(un_entity), dtype=int))                    #  尾实体发生时间
            rel_h.append(np.concatenate((u1 + idx[i], u2 + idx[j])))
...
if __name__ == '__main__':
    ...
    print(args)
    # dataset_folder = f"../bertintkg/data/{args.data}"
    ...
    save_path = './' + '_all_graph_' + args.data
    graph, s_index, s_t, s_f, s_l = generate_graph_all(total_data, total_times, time_num=time_num, nodes_num=num_nodes,
